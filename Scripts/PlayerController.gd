...
func _ready() -> void:
	...
	if is_local_player():
		...
		#battery_chamber_r.visible = false # Hide the right battery chamber from the local player
	else:
...
# Other input methods remain unchanged
...
#
## Method to determine if this is the local player
#func is_local_player() -> bool:  # <-- Added this method
	#return multiplayer.get_unique_id() == name.to_int()  # <-- Added this line
#
## Called when the node enters the scene tree for the first time.
#func _ready() -> void:
	#set_process(false) # Stop from _process being called while Player is initalizing
	#self.global_transform.origin = Vector3(-6, 1, -111) # Spawn the player at position (-6, 1, -111)
	#playerInputChecker = PlayerInputChecker.new(self) # Initialize PlayerInputChecker and pass self
	#playerCameraMovement = PlayerCameraMovement.new(self, 0.5) # Initialize PlayerCameraMovement and pass self
	#playerDebugging = PlayerDebugging.new(self)
	#playerStateController = PlayerStateController.new(self)
	#var thirdPersonMesh = get_node("Neck/SAS/Armature/Skeleton3D/Cylinder_001")
	#
	## Local player-specific setup
	#if is_local_player():  # <-- Added this condition
		#thirdPersonMesh.layers = 1 << 2 # Set the mesh layer for local player culling
		#$Neck/Camera3D.cull_mask &= ~(1 << 2) # Adjust the cull mask to hide the local player's mesh
	#set_process(true) # Begin _process being called, initializing finished
#
## Called every frame. 'delta' is the elapsed time since the previous frame.
#func _process(delta : float) -> void:
...
#func _ready() -> void:
	...
	#playerStateController = PlayerStateController.new(self);
	#var thirdPersonMesh = get_node("Neck/SAS/Armature/Skeleton3D/Cylinder_001");
	##thirdPersonMesh.layers = 1 << 2;
	##local_camera.cull_mask &= ~(1 << 2);
	#
	#set_process(true); # Begin _process being called, initializing finished
#
## Called every frame. 'delta' is the elapsed time since the previous frame.
#func _process(delta : float) -> void:
	#pass;
	##playerRPCSynchronizer.rpc("UpdateStateRPC");
#
## Called before all the physics calculations
## NOTE: Capped at 60 FPS
#func _physics_process(delta : float) -> void:
